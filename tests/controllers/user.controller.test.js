const {mockRequest, mockResponce} = require('../intercepter');
...
describe("Update", () => {
    it('should fail if no user found', async () => {
        ...
        expect(userSpy).toHaveBeenCalledWith(200)
        expect(res.status).toHaveBeenCalledWith({
            message: 'No user with id found'
        })
    })
    it('should fail due to internal error', async () => {
        const userSpy = jest.spyOn(User, 'findOneAndUpdate').mockRetunValue(cb => cb(new Error("This is an error")))
        const req = mockRequest();
        const res = mockResponce();
        req.params = {userId: 1};
        await update(req, res)
        expect(userSpy).toHaveBeenCalledWith(500)
        expect(res.status).toHaveBeenCalledWith({
